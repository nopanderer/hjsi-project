...
import hjsi.common.CameraManager;
...
public class Mob
{
    ...
    public void draw(Canvas canvas)
    {
        ...
        canvas.drawCircle(x - CameraManager.getInstance().x() + 32, y - CameraManager.getInstance().y() + 32, range, showRange);
        canvas.drawBitmap(AppManager.getInstance().getBitmap("mob" + wave), x - CameraManager.getInstance().x(), y - CameraManager.getInstance().y(), null);
    }
    ...
    public void move()
    {
        ...
        // 아래로
        if (x == oldX && y + step <= CameraManager.getInstance().worldHeight() - 900)
        {
            y += step;
        }
        // 오른쪽
        else if (x + step <= CameraManager.getInstance().worldWidth() - 1500 && y == CameraManager.getInstance().worldHeight() - 900)
        {
            x += step;
        }
        // 위로
        else if (x == CameraManager.getInstance().worldWidth() - 1500 && y - step >= oldY)
        {
            y -= step;
        }
        // 왼쪽
