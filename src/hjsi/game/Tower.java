...
public class Tower extends Unit implements Attackable
{
    ...
    public void attack()
    {
        ...
        for (int i = 0; i < GameState.getInstance().Mobs.size(); i++)
        {
            ...
            else if ((int) Math.sqrt(Math.pow(GameState.getInstance().Mobs.get(i).cntrX - this.cntrX, 2) + Math.pow(GameState.getInstance().Mobs.get(i).cntrY - this.cntrY, 2)) <= range)
            {
                GameState.getInstance().projs.add(new Projectile(cntrX, cntrY, i, AppManager.getInstance().getBitmap("proj1")));
                break;
            }
        }
